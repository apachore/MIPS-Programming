...
# Your CODE HERE
# a0, a1, a2 - counters
# LP1 - loop for traversing along the row of matrix A
# LP2 - loop for traversing along the column of matrix B
# LP3 - for changing the row of matrix A
		mul	$t7, $s1, 4 #constant 4*n value store which used for calculating memory locations
		la	$t2, matrix_c
		la	$t0, matrix_a
		addi	$a2, $0, 0
LP3:		bge	$a2, $s1, mult_end
		addi	$a0, $0, 0
LP2:		bge	$a0, $s1, last_loop
		mul	$t8, $a0, 4
		la	$t1, matrix_b
		add	$t1, $t1, $t8
		addi	$t6, $0, 0
